...
const AdminDoc = () => {
  ...
  <div>
      ...
      <Container text="Documentation">
        ...
        <div style={{ width: `95%`, height: '60vh', display: 'flex'}} className="admin-small-container">
            ...
            {info.map((data, index) => {
              return (
                <div key={index} style={{paddingBottom: '1.5em', }}>
                  ...
                  <button 
                    style={{backgroundColor: 'transparent', border: 'none', fontSize: '1.6em', cursor: 'pointer'}}
                    onClick={() => toggleSubheadingsVisibility(index)}>
                    {subheadingsVisibility[index] ? '▾' : '▸'} {data["heading"]}
                  </button>

                  {subheadingsVisibility[index] && 
                    <div style={{marginLeft: '3em', fontSize: '1.2em', display: 'flex', flexDirection: 'column', gap: '0.3em', marginTop: '0.3em', alignItems: 'start'}}>
                      {data["subheadings"].map((subdata, indexTwo) => {
                        return (
                          <button key={indexTwo}
                            style={{backgroundColor: 'transparent', border: 'none', fontSize: '1em', margin: 0, cursor: 'pointer', opacity: '0.6'}}
          ...
          <div style={{width: '70%', borderLeft: 'solid', borderWidth: '2px', borderColor: 'rgb(200, 200, 200)', paddingLeft: '3em', paddingTop: '2em', paddingBottom: '2em'}}>
            <ReactMarkdown 
              remarkPlugins={[remarkGfm]}
              components={{
                h1: ({node, ...props}) => <h1 style={{fontSize: '1.6em'}} {...props}/>,
                h2: ({node, ...props}) => <h2 style={{fontSize: '1.5em'}} {...props}/>,
                h3: ({node, ...props}) => <h3 style={{fontSize: '1.4em'}} {...props}/>,
                h4: ({node, ...props}) => <h4 style={{fontSize: '1.3em'}} {...props}/>,
